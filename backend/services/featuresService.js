import { interview_panel, law_makers, medical_specialists } from "../PreBuildAgents.js";
...
function scoreByKeywords(agent, keywords = [], weightedMatches = {}) {
  ...
  Object.entries(weightedMatches).forEach(([keyword, bonus]) => {
    if (blob.includes(keyword)) score += bonus;
  });
...
function ensureDiverseFallback(agents, pool, count) {
  ...
  const usedIds = new Set(agents.map((agent) => agent.id));
  const fillers = pool.filter((agent) => !usedIds.has(agent.id)).slice(0, count - agents.length);
...
function generateLawPanel(topic) {
  ...
  const scored = law_makers.map((agent) => ({
    ...
    score: scoreByKeywords(
      agent,
      topicLower.split(/\W+/).filter(Boolean),
      {
        constitution: 5,
        constitutional: 5,
        rights: 4,
  ...
  const experts = ensureDiverseFallback(
    ranked.filter((agent) => agent.score > 0).map(trimScore),
    ranked.map(trimScore),
    5
  );

  const judges = experts.filter((agent) => /justice|judge|chief justice/i.test(agent.role));
  const advocates = experts.filter((agent) => !judges.some((judge) => judge.id === agent.id));
...
function generateInterviewPanel(scenario) {
  ...
  const interviewers = ensureDiverseFallback(
    sortByScore(scored)
      .filter((agent) => agent.score > 0)
      .map(trimScore),
    sortByScore(scored).map(trimScore),
    5
  );
...
function generateMedicalPanel(medicalCase) {
  ...
  const scored = medical_specialists.map((agent) => ({
    ...agent,
    score: scoreByKeywords(agent, keywords, specialtyBonuses),
  }));

  const doctors = ensureDiverseFallback(
    sortByScore(scored)
      .filter((agent) => agent.score > 0)
      .map(trimScore),
    sortByScore(scored).map(trimScore),
